...
function PluginManager(rootFolder) {
    ...
    this.executeCommand = async (command) => {
        ...
        if (typeof res === "undefined") {
            ret.operationType = 'sync';
            ret.result = undefined;
            return ret;
        }
        // Updated response type handling
        if (res.constructor.name === 'SlowResponse') {
            ret.operationType = 'slowLambda';
            ret.result = res.getCallId();
        } else if (res.constructor.name === 'ObservableResponse') {
            ret.operationType = 'observableLambda';
            ret.result = res.getCallId();
        } else {
            ret.operationType = 'sync';
            ret.result = res;
