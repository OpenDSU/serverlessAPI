...
function PluginManager(rootFolder) {
    ...
    this.init = async () => {
        ...
        // Load plugin modules and map them by name
        const pluginModules = {};
        ...
        for (const pluginFile of pluginFiles) {
            try {
                ...
                // Skip files that don't export a getName function
                if (typeof pluginModule.getName !== 'function') {
                    console.warn(`Plugin file ${pluginFile} doesn't export a getName function, skipping`);
                    continue;
                }
                
                const pluginName = pluginModule.getName();
        ...
        for (const pluginName of sortedPlugins) {
            try {
                ...
                const pluginFile = pluginFiles.find(file => {
                    try {
                        const module = loadPluginModule(file);
                        return module.getName && module.getName() === pluginName;
                    } catch (e) {
                        return false;
                    }
                });
